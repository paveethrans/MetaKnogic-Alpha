...
async def _merge_hyperedges_then_upsert(
    ...
    try:
        ...
        logger.warning(
            f"Skip get_node for hyperedge '{hyperedge_name}' due to error: {e}"
        )
        already_hyperedge = None
    ...
    try:
        ...
        logger.warning(
            f"Skip upsert_node for hyperedge '{hyperedge_name}' due to error: {e}"
        )
...
async def _merge_nodes_then_upsert(
    ...
    try:
        ...
        logger.warning(f"Skip get_node for entity '{entity_name}' due to error: {e}")
        already_node = None
    ...
    try:
        ...
        logger.warning(
            f"Skip upsert_node for entity '{entity_name}' due to error: {e}"
        )
...
async def _merge_edges_then_upsert(
    ...
    for node in nodes_data:
        ...
        try:
            has = await knowledge_graph_inst.has_edge(hyper_relation, entity_name)
        except Exception as e:
            logger.warning(
                f"Skip edge existence check for '{hyper_relation}' -> '{entity_name}' due to error: {e}"
            )
            has = False
        if has:
            try:
                ...
                logger.warning(
                    f"Skip get_edge for '{hyper_relation}' -> '{entity_name}' due to error: {e}"
                )
                already_edge = None
        ...
        try:
            ...
            logger.warning(
                f"Skip upsert_edge for '{hyper_relation}' -> '{entity_name}' due to error: {e}"
            )
            continue
...
async def kg_query(
    ...
    if not hl_keywords and not ll_keywords:
        ...
        return PROMPTS["fail_response"]
    ll_joined = ", ".join(ll_keywords)
    hl_joined = ", ".join(hl_keywords)

    # In hybrid mode, degrade gracefully to the available signal
    used_query_param = query_param
    if query_param.mode in ["hybrid"]:
        if not ll_joined and hl_joined:
            logger.warning("low_level_keywords is empty; switching to global mode")
            try:
                used_query_param = QueryParam(**{**vars(query_param), "mode": "global"})
            except Exception:
                # fallback: mutate if dataclass semantics are unavailable
                used_query_param.mode = "global"
        elif not hl_joined and ll_joined:
            logger.warning("high_level_keywords is empty; switching to local mode")
            try:
                used_query_param = QueryParam(**{**vars(query_param), "mode": "local"})
            except Exception:
                used_query_param.mode = "local"

    # Build context
    keywords = [ll_joined, hl_joined]
    ...
    if used_query_param.only_need_context:
        return context
    if context is None:
        return PROMPTS["fail_response"]
    sys_prompt_temp = PROMPTS["rag_response"]
    sys_prompt = sys_prompt_temp.format(
        context_data=context, response_type=used_query_param.response_type
    )
    # Safety clamp: ensure final system prompt stays within provider token limits
    try:
        t_model = global_config.get("tiktoken_model_name", "gpt-4o-mini")
        sys_tokens = encode_string_by_tiktoken(sys_prompt, model_name=t_model)
        max_prompt_tokens = int(global_config.get("max_prompt_tokens", 240000))
        if len(sys_tokens) > max_prompt_tokens:
            sys_prompt = decode_tokens_by_tiktoken(
                sys_tokens[:max_prompt_tokens], model_name=t_model
            )
    except Exception:
        # If tokenization fails for any reason, proceed without clamping
        pass
    if used_query_param.only_need_prompt:
        return sys_prompt
    response = await use_model_func(
        query,
        system_prompt=sys_prompt,
        stream=used_query_param.stream,
    )
...
async def _get_node_data(
    ...
    # Prefer static ids if available (Milvus returns 'id'), otherwise use names
    _entity_ids_or_names = [r.get("id", r["entity_name"]) for r in results]
    node_datas = await asyncio.gather(
        *[knowledge_graph_inst.get_node(x) for x in _entity_ids_or_names]
    )
    ...
    node_degrees = await asyncio.gather(
        *[knowledge_graph_inst.node_degree(x) for x in _entity_ids_or_names]
    )
    node_datas = [
    ...
    # build prompt
    entites_section_list = [["id", "entity", "type", "description", "paper_id"]]
    ...
    relations_section_list = [
        ["id", "hyperedge", "related_entities", "paper_id"]
    ]
...
async def _find_most_related_text_unit_from_entities(
    ...
    for this_edges in edges:
        ...
        all_one_hop_nodes.update(
            [e[1] for e in this_edges if e is not None and len(e) > 1 and e[1]]
        )
...
async def _find_most_related_edges_from_entities(
    ...
    for this_edges in all_related_edges:
        for e in this_edges or []:
            if not e or len(e) < 2 or not e[0] or not e[1]:
                continue
            sorted_edge = tuple(e)
    ...
    # Attach paper_id from the hyperedge node if available (k[1] is hyperedge label)
    try:
        hyper_nodes = await asyncio.gather(
            *[knowledge_graph_inst.get_node(e["src_tgt"][1]) for e in all_edges_data]
        )
        all_edges_data = [
            {**e, "paper_id": (hn.get("paper_id", "") if hn else "")}
            for e, hn in zip(all_edges_data, hyper_nodes)
        ]
    except Exception:
        pass
    ...
    for this_nodes in all_related_nodes:
        if not this_nodes:
            all_nodes.append("")
            continue
        safe = [str(n[1]) for n in this_nodes if n is not None and len(n) > 1 and n[1]]
        all_nodes.append("|".join(safe))
    all_edges_data = [
        {**e, "related_nodes": n}
...
async def _get_edge_data(
    ...
    _edge_ids_or_names = [r.get("id", r["hyperedge_name"]) for r in results]
    edge_datas = await asyncio.gather(
        *[knowledge_graph_inst.get_node(x) for x in _edge_ids_or_names]
    )
...
async def _find_most_related_entities_from_relationships(
    ...
    for node_data in node_datas:
        for e in node_data or []:
            if not e or len(e) < 2 or not e[1]:
                continue
            if e[1] not in seen:
    ...
    # Ensure defaults and skip missing nodes; always provide 'description'
    safe_node_datas = []
    for k, n, d in zip(entity_names, node_datas, node_degrees):
        if n is None:
            continue
        desc = n.get("description", n.get("entity_name", ""))
        safe_node_datas.append({**n, "entity_name": k, "rank": d, "description": desc})
    node_datas = safe_node_datas
